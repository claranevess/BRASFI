let addAulaTrilha = null; // Variável para armazenar a referência ao modal

async function loadaddAulaTrilha() {
    if (!addAulaTrilha) { // Carrega o modal apenas uma vez
        try {
            ...
            document.getElementById('addAulaTrilhaContainer').innerHTML = htmlContent;

            addAulaTrilha = document.getElementById('modalAdicionarAula'); // ID do modal de adicionar
            const closeAddAulaBtn = addAulaTrilha.querySelector('#closeModal');

            if (closeAddAulaBtn) {
                closeAddAulaBtn.addEventListener('click', closeaddAulaTrilha);
            }
            if (addAulaTrilha) {
                addAulaTrilha.addEventListener('click', (e) => {
                    if (e.target === addAulaTrilha) {
                        closeaddAulaTrilha();
                    }
                });
            }
...
function closeAddAulaModal() {
    if (addAulaTrilha) {
        addAulaTrilha.classList.add('hidden');
        resetForm();
    } else {
...
async function showAddAulaModal() {
    await loadaddAulaTrilha(); // Garante que o modal seja carregado
    if (addAulaTrilha) {
        addAulaTrilha.classList.remove('hidden');
